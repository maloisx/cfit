...
select nom_disciplina as name , count(activo) as y
FROM
(
SELECT
#v.cod_venta_membresia,
...
v.fecha_inicio_plan,
DATE_ADD(DATE_ADD(v.fecha_inicio_plan,INTERVAL p.duracion_meses MONTH) , INTERVAL p.duracion_dias DAY ) as fecha_fin_plan,
DATEDIFF(DATE_ADD(DATE_ADD(v.fecha_inicio_plan,INTERVAL p.duracion_meses MONTH) , INTERVAL p.duracion_dias DAY ) , now()) as dias_diff
FROM
venta_membresia AS v
INNER JOIN plan AS p ON v.cod_plan = p.cod_plan
INNER JOIN disciplina AS d ON p.cod_disciplina = d.cod_disciplina
...
WHERE
v.cod_estado = 1
and DATEDIFF(DATE_ADD(DATE_ADD(v.fecha_inicio_plan,INTERVAL p.duracion_meses MONTH) , INTERVAL p.duracion_dias DAY ) , now())  BETWEEN 0 and 30

order by dias_diff asc
